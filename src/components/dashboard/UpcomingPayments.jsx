...
import { Calendar, AlertCircle, Eye, CheckCircle2, ShieldCheck, CreditCard, ArrowRight } from 'lucide-react'
...
export function UpcomingPayments({ liveInvoices = [] }) {
  ...
  const items = useMemo(() => {
    ...
    const queueInvoices = liveInvoices.filter((i) => i.status === 'PAYMENT_QUEUE' || i.status === 'Approved')
    return queueInvoices.slice(0, 3).map((inv) => ({
      id: inv._id,
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        ...
        <Link
          ...
          className="flex items-center gap-1 text-xs font-extrabold text-blue-600 hover:text-blue-700 transition"
        >
      ...
      <h3 className="mt-1 text-base font-bold text-slate-900">Approved Payments Queue</h3>
          ...
          items.map((row) => (
            ...
            >
              <div className="flex items-center gap-3">
                <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-50 font-bold text-indigo-600 text-xs">
                  {row.vendor[0]}
                </div>
                <div>
              ...
              <div className="flex flex-wrap items-center justify-between sm:justify-end gap-2 text-xs">
                <span className="flex items-center gap-1 rounded-md bg-indigo-50 border border-indigo-200 px-2 py-0.5 font-bold text-indigo-700 text-[11px]">
